...
Although already available on \href{https://cran.r-project.org/web/packages/shinystan/index.html}{CRAN} as a functional prototype,  \textit{shinystan} will need to undergo considerable improvements of the interface and under the hood during this project periods. Posterior predictive checking is one area where the need for additional options and functionality will be obvious to the sophisticated data scientists \cite{shinystan}. We also want to expand  the existing collaborative model sharing via the internet in the project period.
